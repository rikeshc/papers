...
Similarly I appreciate the PhD students in the department for answering my stupid questions without hesitation. I should also remember my fellow students who were very engaging in discussing my problems and suggesting creative ideas. \\
...
\begin{figure} [H]
  \centering
  \includegraphics[width=\textwidth]{img/pms/pms_Vs_both.pdf} 
...
\begin{figure} [H]
  \centering
  \includegraphics[width=\textwidth]{img/pms/pms_Vout_both.pdf} 
...
\begin{figure} [H]
  \centering
  \includegraphics[width=\textwidth]{img/pms/pms_ripple_both.pdf} 
 \caption{Ripple in Vrec and Vreg} 
\label{fig:pms_ripple} 
\end{figure}

Figure \ref{fig:pms_PI} shows transient waveform of diode currents corresponding to its input voltages. The results for both 
pre and post layout simulation are similar. It is seen that diodes turn on slightly earlier and turn off slightly later. 
...
Vin1 and Vin2. Similarly Irms is less in post layout because diode current is dependent on Vin1 and Vin2 and in post 
layout Vin1 and Vin2 is always less than in schematic due to path resistance from pad to the diodes.\\


\begin{figure} [H]
  \centering
  \includegraphics[width=\textwidth]{img/pms/pms_VIrect_both.pdf} 
 \caption{Current and power consumption} 
\label{fig:pms_PI} 
\end{figure}

The total power consumed from the source are 37 mW and 33 mW for pre and post layout simulation for delivering 17.8 mW to 
the load. The input power to the rectifier is 28.7 mW and 27.2 mW respectively which is obtained after subtracting power 
consumed by the source resistance. The increase in post layout efficiency is explained by two things: reduced Irms of 
...
This results in power transfer efficiency, $\eta_{pms}$ of 62 and 65.5 respectively. In 
...
Using $\eta_{rect}$ from table \ref{tab:rect_spec} and $\eta_{ldo}$ from table \ref{tab:ldo_spec}, the pre and post layout efficiencies are this and this respectively 
which are close match with the obtained values here. \\
...
\hline \hline
			& \textbf{Schematic}	& \textbf{Post-layout} 	\\
\hline \hline
$V_{rec}$ 			& 2.32 \si{\volt}		& 2.26 V	\\ \hline
...
efficiency, $\eta_{wpt}$, of 57.1\% and 59.4 \% respectivley. This $\eta_{wpt}$ of complete WPT system should be equal to 
...
PMS simulation. The average rectified output $V_{rec}$ is a close to simulated value. The additional drop in measured $V_{rec}$
 can be accounted to resistive drop along bonding wire and PCB trace. The differential input waveforms are similar in shape 
...
of LDO. Yet, because of higher quiescent current, no useful power can be delivered to the load. Hence for the discussion ahead, rectifier is only considered as PMS.
