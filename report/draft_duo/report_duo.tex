...
\usepackage[toc, page]{appendix} % appendicex
\usepackage{adjustbox} %rotate caption of image
...
\lstset{language=Matlab,
   ...
   breaklines = true, %del opp linjer dersom de skulle flyte utenfor ramma
   frame=shadowbox, rulesepcolor=\color{black}} %ramme rundt koden
%Ramma gjør det lettere å skille hva som er kode og hva som er kommentarer.
...
\newacronym{a4wp}{A4WP}{Alliance for Wireless Power}
...
already available power? We will discuss it ahead. 
...
\begin{figure} [htbp]
  \begin{minipage}{\linewidth}
  \centering 
  \subfloat[Qualcomm EV charger]  {\includegraphics[width=.49\textwidth]{img/wpt_ev_qualcom.jpg} \label{wpt_ev}}
  \hfill
 \subfloat[Qualcomm mobile phone charger with foreign object detection feature]  {\includegraphics[width=.49\textwidth]{img/wpt_mobile_qualcom.jpg}\label{wpt_mobile}}
 \hfill
 \end{minipage}\par\medskip
 \centering
 \subfloat[IDT concept of wireless charging of bionic devices]  {\includegraphics[width=.5\textwidth]{img/wpt_bionics.jpg}\label{wpt_medical}}
...
\subsubsection{A4WP}
\acrshort{a4wp} is  another independent wireless 
charging standard. It uses magnetic resonance IPT technique for power transfer. The main differences between 
...
wearable is taken as a reference load for this project.  The purpose of this work will be to make a wireless power transfer system for cordlessly driving
a load or charging a battery of 1.8 V with 10 mA current. The target specification for this work is listed in table \ref{tab:proj_spec_tar}. \\
...
There has been a lot of similar works in inductive wireless power transfer. With recent trend of integrating wireless power transfer in consumer electronics, application specific optimisation has been abundantly studied. \cite{wpt_3r_rect} has presented  wireless power transfer system for implantable medical devices implemented using 0.35 um CMOS process and operating at 13.56 MHz, proposing techniques to minimise load and coupling variation. Similarly \cite{wpt_high_power} has proposed high power transfer receiver with magnetic resonance coupling operating at 6.87 MHz fabricated in 0.13 um BCD process. \\

Besides complete WPT systems, there are also numerous works in optimising individual module of WPT systems. For example \cite{hf_band_matching} discusses about effect of impedance mismatch in transfer link in high frequency wireless power transfer system. Likewise, \cite{rectrcc} and \cite{parasitic_effect} discuss about high efficiency active rectifier topologies for wireless power transfer. Low loss CMOS active rectifier in \cite{rectrcc} has been implemented in this work.  Apart from being reference to this project, these works also helped in explaining and reasoning  the design issues encountered in this work. 
...
Figure \ref{rcc}  is the implementation of four input comparator $D2$ used in \ref{rect_rcc} as proposed in \cite{rectrcc}. It is designed to self power and bias because no steady state supply is available at start up. $M1$, $M2$ and $M7$ monitors voltage across $Mn4$ i.e $Vin2 - Vgnd$ and $M3$, $M4$ and $M8$ monitors voltage across $Mp3$ ie $Vin1 - Vrec$ . So when $Vin1 - Vrec > Vin2 - Vgnd$ which means $Vin > Vrec$, output of $D2$ is high and turns on $Mn4$ instantly. But when $Vin < Vrec$, the output of comparator is delayed to fall which causes $Mn4$ to conduct in reverse direction leading to significant reduction in power delivered to load. $M8$ is introduced in order to overcome this problem which adds offset currents to increase $Van$ and $Vpn$ faster, causing the output to decrease faster and turns off $Mn4$ before $Vin < Vrec$. This reverse current control technique compensates the comparator delay and increases the power efficiency of the rectifier. \\

The design parameters for the rectifier is listed in table  \ref{tab:rect_parameter}. The dimensions of the pass devices are first hand calculated by using square law current equation and devices parameters values given in the technology documents, and later optimised with simulation tool in order to make the rectifier to deliver the required current. Since \acrshort{nmos} does not have to have same device size as \acrshort{pmos} to deliver same current, optimal size ratio equation from \cite{rectsize} is used to find nMOS pass devices sizes. Thought maximum load for this work is 10 mA, It is always simulated with 0.5 mA extra load. This extra current is to account for the fact that RCC is self powered and LDO which will follow this rectifier will be powered by $Vrec$. Similarly, the value of ripple rejection capacitor is chosen 100nF. This size of 
...
Figure \ref{rect_tb} is the test bench setup for simulation of the rectifier. Figure  \ref{fig:rect_v_post}  presents both pre and post layout results of input voltages, $Vin1$ and $Vin2$  and output voltage, $Vrec$, for one cycle of ac input. The closer view of rectified output is shown in figure \ref{fig:rect_ripple}. The voltage drop of about 60 mV in layout result is accounted for the voltage drop due to 
...
Figure \ref{rect_plot} is the simulation results showing voltages at rectifier inputs and output rectified voltage along with corresponding current waveform through rectifying MOSes. On comparing schematic and post-layout results, two important observations can be made from plots. First is reduction in peak diode current in post layout result. This is due to in addition to channel resistance of rectifying diodes, there is additional resistance of high current conducting path in the layout. In schematic, the paths are ideal wire with no resistance and hence higher peak current. Second is reverse leakage current is more pronounced in post layout simulation.   It is because wide path run over drain and source of rectifying diodes, creating larger parasitic capacitance. This is causing slight delay in turning off the diodes.\\

\begin{figure}[htb] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=.95\textwidth]{img/rectifier_V_both_new.pdf} 
...
\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=0.95\textwidth]{img/rectifier_ripple_both_new.pdf} 
   \caption{Rectified output for pre and post layout}
   \label{fig:rect_ripple}
\end{figure}

\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/rectifier_VI_both_new.pdf} 
...
VCE, ratio of rectified DC, $Vrec$ to peak ac input, |$Vin$| with respect to magnitude peak ac input signal. |$Vin$| is gradually increased in 
...
\hline \hline
Rectified DC	 	& 2.28 V 				& 2.14 V	\\ \hline
Ripple \acrshort{vpp} & 3.1 mV 				& 2.8 mV	\\ \hline
Peak diode current 	& 63 mA 			&	57.3 mA	\\ \hline
PCE 				& 84.5 \% 				&	82.9 \%	\\ \hline
VCE 				& 88.6 \% 				&	86.2 \%	\\
\hline \hline
\end{tabular}
...
here which is turn increase the accuracy of regulated voltage, $V_{reg}$ \cite{ldo_bulkmod}. Typically error amplifier 
...
Reference voltage, V\textsubscript{ref} will be band-gap voltage, 1.18 V, of silicon and  thus \acrshort{icmr} 
...
\begin{figure} [!htbp]
  \centering 
  %\subfloat[Inductive coupling between two coils]  {\includegraphics[width=.49\textwidth]{img/ant_induction.pdf} \label{fig:ant_induction1}}
  \subfloat[Inductive coupling between two coils]  {\includegraphics[width=.49\textwidth]{img/visio/visio_induction.pdf} \label{fig:ant_induction1}}
\hfill
 \subfloat[Model of inductive coupling]  {\includegraphics[width=.49\textwidth]{img/ant_induction_model.pdf}\label{fig:ant_induction2}}
...
\hfill
 ...
 \subfloat[Equivalent schematic]  {\includegraphics[width=.49\textwidth]{img/visio/visio_antenna_single.pdf}\label{fig:ant_single_schematic}}
...
\hfill
 ...
 \subfloat[Equivalent schematic]  {\includegraphics[width=.49\textwidth]{img/visio/visio_antenna_couple.pdf}\label{fig:ant_couple_schematic}}
...
\begin{figure}[!htb] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/visio/visio_antenna_couple_resonant.pdf} 
...
\begin{figure} [!htb]
  \centering
  \includegraphics[width=0.95\textwidth]{img/ant_s1_s2_both.pdf} 
...
\begin{figure} [H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/ant_s21_both.pdf}
...
$Freq.$							& 13.56				& 13.56				& 13.56				& \si{\mega\hertz}	\\ \hline
...
\begin{figure} [!htb]
  ...
  \includegraphics[width=0.9\textwidth]{img/visio/visio_wpt_func_block.pdf} 
...
Figure \ref{fig:wpt_top} is the top level of PMS in this design and test bench setup is a shown in figure \ref{fig:wpt_top_testbench}. The purpose here is to give differential signal $V_{in1}$ and $V_{in2}$ to PMS system and see $V_{rec}$ and $V_{reg}$ outputs while driving load. The voltage signal from  source $V_{ac}$ is fed to balun in order to generated $V_{in1}$ and $V_{in2}$. The other inputs are external biases, reference, control and supply for LDO and buffer. Other outputs are bias voltages to examine the performance of BGR circuit which can be disabled with external control signal Vctl low.  \\
...
\begin{figure} [H]
  \centering
  \includegraphics[width=1\textwidth]{img/visio/visio_pms_top.pdf} 
...
Figure \ref{fig:pms_vin} and \ref{fig:pms_vout} shows input and output waveforms of PMS block when supplying 10 mA current to the load. On comparing $V_{rec}$ with 
that of rectifier only section, two distinct difference can be observed: additional drop in $V_{rec}$ and additional time to 
get maximum $V_{rec}$. As already mentioned that final parasitic extraction from layout not just include the core but also 
the tracks from pad to the core inputs, which obviously contributes more resistance in the conduction path and hence 
more drop in $V_{rec}$. Similarly, passive and active rectification region is clearly seen here. $C_{load}$ being a large 
capacitor, it is taking longer time to start the active rectification. The regulated output, $V_{reg}$ is similar to as discussed in LDO section. \\

\begin{figure} [!htb]
  \centering
  \includegraphics[width=1\textwidth]{img/pms/pms2_Vs_both.pdf} 
...
\begin{figure} [!htb]
  \centering
  \includegraphics[width=0.95\textwidth]{img/pms/pms2_Vout_both.pdf} 
 \caption{$V_{rec}$ and $V_{reg}$ voltages of PMS} 
\label{fig:pms_vout} 
\end{figure}

\begin{figure} [H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/pms/pms2_ripple_both.pdf} 
 \caption{Ripple in $V_{rec}$ and $V_{reg}$} 
\label{fig:pms_ripple} 
\end{figure}

Figure \ref{fig:pms_ripple} gives a zoomed view of rectified, $V_{rec}$ and regulated, $V_{reg}$ output voltages. Their 
characteristics are same as discussed in rectifier and LDO section. \\
...
Same is the reason for rms diode current being slightly less in post layout. The other important observation is peak diode though $V_{in1}$ path is 
less than through $V_{in2}$ path which means paths from pad to diode inputs are asymmetrical with $V_{in1}$ track having higher resistance.\\
...
%Figure \ref{fig:wpt_top_link} is test bench for complete PRU unit. It includes PMS shown in figure \ref{fig:wpt_top} and resonant inductive link shown in figure \ref{fig:ant_couple}. The inductive link used here is lumped model of coupling extracted from HFSS for the case of 5 mm separation between the coils where both antennas are tuned at the operating frequency, 13.56 MHz using capacitors $Cp1$, $Cp2$ and $Cs1$ given in table \ref{tab:ant_couple_parameter}. The primary antenna is driven by voltage signal source  and the secondary antenna generate differential signal  and Vin2, which are inputs to the PMS chip. \\

Figure \ref{fig:wpt_top_testbench} is also the test bench for complete WPT PRU unit when the balun block is replaced with wireless transfer link model. Figure \ref{fig:wpt_top_link} is the transfer link realised with lumped model of resonant inductive link. The values of lumped elements of inductive link and tuning capacitors 
are given in table  \ref{tab:ant_inductance_compare}, \ref{tab:ant_couple_parameter} and \ref{tab:ant_spec}. $V_{ac}$, a 13.56 MHz voltage source, is connected to $V_{pri}$ input of this model and differential voltages $V_{sec+}$ and $V_{sec-}$ are generated as output, which are used as $V_{in1}$ and $V_{in2}$ inputs for PMS system. The simulation result of 5 mm distance coupling is presented below. \\

Since the nominal peak to peak value of differential inputs is 2.6V, during simulation magnitude of $V_{ac}$ is gradually increased till the differential signals in secondary antenna reach the nominal value when driving maximum load. Other biasing voltages and component 
...
\begin{figure} [H]
  \centering
  \includegraphics[width=1\textwidth]{img/visio/visio_res_ind_link.pdf} 
 \caption{Lumped model of inductive link } 
\label{fig:wpt_top_link} 
\end{figure}
...
\begin{figure} [!htb]
  \centering
  \includegraphics[width=0.95\textwidth]{img/wpt/wpt_Vout_both.pdf} 
 \caption{$V_{rec}$ and $V_{reg}$ voltages} 
\label{fig:wpt_vout} 
\end{figure}
...
\begin{figure} [H]
  \centering
  \includegraphics[width=0.95\textwidth]{img/wpt/wpt_ripple_both.pdf} 
 \caption{Ripple in $V_{rec}$ and $V_{reg}$} 
...
current. This is because $V_{in} (V_{in1} - V_{in2})$  changes quickly during turning on time of diodes. That is why the peak of Vin 
is more like a square wave. Similarly as in PMS section, diode in $V_{in1}$ path is conducting less peak current than in $V_{in2}$ path due to two 
...
\subsubsection{All inductive links}
...
measure current where required. All these devices are connected to a GPIB bus and controlled using Matlab script. An example of Matlab code used here is given in the appendix.
...
wireless power transfer. The WPT chip was produced using 2.5 V devices of 90 nm CMOS process. The target performance was to wirelessly power 10 mA load with 1.8 V.   \\

The post-layout simulation result, summarised in table \ref{tab:wpt_spec}, showed that the target objective of WPT system implemented in this project can be achieved with system efficiency greater than 50 \%. Measured performance of rectifier was decent, the limitation being effect of parasitic inductance and capacitance.  Similarly, in spite of higher drop and leakage, LDO measured performance showed that its functionality agreed with the simulation result. The inductive link power transfer efficiency for strong and moderate coupling was reasonable good enough to give measured WPT system efficiency of 46.1 \% taking rectifier only as PMS. \\

 To sum up the work, it can be said that the conceptual design of WPT implemented in this work was approved by post layout simulation,  with measured results backing the simulated performance. The analysis of degraded measured results were traced to layout design of PMS and effect of parasitics. \\
...
\begin{itemize}
\item \textbf{Device size:} I followed a text book approach for designing which I later realised not to be an optimal design choice. For example, choosing device length 3 times the minimum feature size. Though simulation across all corners 
...
\item \textbf{Layout:} During verification  of the chip, larger ripples, both in $V_{rec}$ and $V_{reg}$ were measured. The main reason for this is rectifier and LDO sharing the same ground path. 
...
\item \textbf{Inductive link:} A more detail analysis of magnetic resonance inductive link is required. The coupling link was designed for fixed resonance frequency at 13.56 MHz but shift in resonance frequency was observed while taking measurements of WPT chip. It was 
...
\item \textbf{PCB:} The test PCB could have been made better in terms of both performance and test connivence. The current conduction paths, $V_{in1}$ and $V_{in2}$ should have been made shorter and symmetrical. The decoupling capacitors and ripple rejection capacitors should be place closer to their corresponding JLCC pins. For making measurements easier, more test points are required, especially current measurements points.
\item \textbf{Test-bench:} The top level test-bench of WPT chip can also be improved to realise a real measurement set up. Inclusion of parasitics of probes increases the accuracy of measured data and hence the verification of performance.  More important is inclusion of pad resistance and capacitance which are the source of voltage/power loss and shift of resonance frequency respectively. Moreover, power down switches should be added to disable rectifier and LDO so that they can be tested separately.
\end{itemize}
...
\chapter{Matlab code for PMS measurement}
