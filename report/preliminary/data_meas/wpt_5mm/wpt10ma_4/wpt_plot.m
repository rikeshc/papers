...
vpri = importdata('C1vac_10ma00000.dat');
vac_drop = importdata('F2vindrop_10ma00000.dat');

Iac = vac_drop(:,2)/5*1000;
time = vpri(:,1)*10^9+212.6; % time in ns
timep = (t_pex-t_pex(1))*10^9+52.7;
...
legend('MEAS: V_{ac}', 'SIM: V_{ac}', 'MEAS: I_{ac}', 'SIM: I_{ac}', 'location', 'best');
...
vac_drop = importdata('F2vindrop_10ma00000.dat');
Iac = vac_drop(:,2)/5*1000;
time = vpri(:,1)*10^9; % time in ns

pwr_sim = 1/(timep(end)-timep(1))*trapz(timep, Iac_pex.*vpri_pex);
pwr_meas = 1/(time(end)-time(1))*trapz(time, Iac.*vpri(:,2));
