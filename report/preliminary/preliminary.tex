...
Vbias4					& 0.68 V \\ \hline
Vref						& 1.17 V \\ \hline
C\textsubscript{load} 		& > 0.6 \si{\micro\farad}  \\ \hline
...
Figure \ref{fig:ldo_tran} is the transient simulations of the LDO which illustrates the generation of  $Vreg$ for 
both maximun load, 10 mA and ligh load, 100 uA condition. Irrespective of load, it takes 32 us to stabilise the regulated output of 1.797 V. For maximum load 
of 10 mA, a bump is observed before settling[WHY]. \\

\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_transient.pdf} 
...
It gives information about how well and how fast regulated output settles for line and load variations. In  figure \ref{fig:ldo_loadr} load is given as pulse
 varying from 10 uA to 10 mA with both falling and rising time of 1 ns. Sudden increase in load causes the output voltage to drop. The error amplifier then 
 adjusts the gate voltage of pass device to low. But since it is a pMOS pass  device, it is taking longer time to fully turn on and it is evidently seen in the 
 plot by taking longer time to recover. Likewise when the load suddenly drops to minimum, it causes the output voltage to increase. Again error amplifier 
 adjust it back by increasing the gate voltage of pass device to turn it off. This time however, recovery is quicker. Similarly for line variation observation, input, 
 $Vrec$ is pulsed from 2 V to 2.75 V with 1ns rising and falling time. Sudden increase in input voltage causes output to increase and vice-versa. As in load variation
  case, similar recovery pattern is seen. One important difference here during low to high line regulation transition is that the output is settled 
  to 4 mV less than before transition. [BECAUSE] \\ 

\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_loadr.pdf} 
...
\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_liner.pdf} 
...
of internal folded cascode error amplifier. However after turning on, it requires only 112 mV drop for proper regulation for maximum load and is even lesser for lighter 
load. The line regulation from the plot is calculated[VALUE].  Similarly from figure  \ref{fig:ldo_Iload} load regulation is calculated [VALUE]. \\ 

\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_dc.pdf} 
...
\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_Iload.pdf} 
...
Figure  \ref{fig:ldo_sta} open loop gain and phase margin of LDO without and with compensation. In the upper  uncompensated, two poles below UGF are seen: 
the first one at 400 KHz due to output resistance of pass device and its parasitic capacitance, and the second one at  50 MHz due to buffer output resistance and gate 
capacitance of pass device. Due to these two poles both occurring below UGF, the PM fallen to -45\textdegree. For making the LDO stable, as discussed 
in the beginning, a large capacitor, $C\textsubscript{load}$ with specific series equivalent resistance, $R\textsubscript{esr}$ is used at the output.  $C\textsubscript{load}$ and pass device output resistance creates the dominant pole at 1 KHz and $R\textsubscript{esr}$ and $C\textsubscript{load}$ creates a right hand zero at UGF which 
cancels the non dominant pole occurring below zero. This eventually makes PM 76\textdegree  and \acrshort{gm} 36 dB. \\ 
...
frequencies. However the ripple rejection is still -42 dB at 13.56 MHz which is decent. As seen in figure \ref{fig:ldo_sta}, the open loop gain of LDO feedback circuit is 
...
\begin{figure}[H] %figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{img/ldo_pssr.pdf} 
   \caption{PSSR performance}
   \label{fig:ldo_pssr}
\end{figure}

Table \ref{ldo_spec} summaries the performance of LDO regulator discussed above. [MORE ON LAYOUT]
...
\hline \hline
				 & \textbf{Schematic}		& \textbf{Post-layout} \\
\hline \hline
PSSR 			 & -42 dB	@ 13.56 MHz	&	\\ \hline
Phase margin		 & 76\textdegree 		&	 \\ \hline
Gain margin		 & 36 dB				&	 \\ \hline
Load regulation 	& 					&	\\ \hline
Line regulation 		&   					& 	\\ 
\hline \hline
\end{tabular}
\end{center}
