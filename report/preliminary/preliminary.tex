...
\cite{ldo_bulkmod} and \cite{ldo_quiescent} are two CMOS implementation of LDO. \cite{ldo_bulkmod} has proposed bulk moudation technique for improving load regulation and stability of capacitorless LDO. Similarly \cite{ldo_quiescent} has  proposed techniques for increasing current efficeincy of conventional LDO. \\

Figure \ref{ldo_cmos} shows the CMOS implementation of LDO in this project. The components in this design includes a folded cascode differential amplifier as error amplifier, pMOS buffer, pMOS pass device and feedback network of resistors. 
...
As briefly mentioned above, the error amplifer amplifies scaled regulated voltage, Vdiv and reference voltage, Vref. It is known that an amplifier with higher open loop \acrshort{dc} gain reduced the closed loop gain error and hence amplifier with higher gain is desired here which is turn increase the accuracy of regulated voltage, Vreg \cite{ldo_bulkmod}. Typically error amplifer has gain $> 40 dB$ which is not achieved with the single stage amiplifier with this technology. Higher gain could have been achieved with multiple single stage but with increased difficulty making the amplifier stable. So for achieving higher DC gain and at the same time for stability convinence, folded cascode amplifier \cite[pp. xx]{razavi_2001} is choosen. 

The The amplifier has a pMOS differential stage in order to obtain lower \acrshort{icmr} of the amplifier closer to gnd because so is Vref. \\
