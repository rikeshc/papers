...
%% Load 
clear all;
load('V_sch.mat'); % data includes 150u to 200u trans
load('V_pex.mat'); % data includes 40u to 80u trans
...
p1 = plot(time2(from: to), Vin1(from: to), 'b--'); hold on;
     plot(time2(from: to), Vin2(from: to), 'b:');
     plot(time2(from: to), Vrec(from: to), 'b-');

hold on;
p2 = plot(t_pex2(from2:to2), V1_pex(from2:to2), 'r--'); hold on;
     plot(t_pex2(from2:to2), V2_pex(from2:to2), 'r:');
     plot(t_pex2(from2:to2), Vrec_pex(from2:to2), 'r-');
...
legend(h, 'Vin1, schematic', 'Vin2, schematic', 'Vrec, schematic', 'Vin1, layout', 'Vin2, layout', 'Vrec, layout', 'location', 'best');
