...
plot(time(from:to), vac_sch(from:to), 'm', time(from:to), vac_rect_sch(from:to), 'r', time(from:to), vin1_sch(from:to), 'g', time(from:to), vin2_sch(from:to), 'b'); hold on;
...
plot(timep(fromp:top), vac_rect_pex(fromp:top), 'r--', timep(fromp:top), vin1_pex(fromp:top), 'g--', timep(fromp:top), vin2_pex(fromp:top), 'b--');
...
print(f1, 'wpt_V_both.pdf', '-dpdf');
movefile('wpt_V_both.pdf','../../img/wpt_V_both.pdf');

set(f2,'Units','Inches');
pos = get(f2,'Position');
set(f2,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(f2, 'wpt_ripple_both.pdf', '-dpdf');
movefile('wpt_ripple_both.pdf','../../img/wpt_ripple_both.pdf');

set(f3,'Units','Inches');
pos = get(f3,'Position');
set(f3,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(f3, 'wpt_inputs_both.pdf', '-dpdf');
movefile('wpt_inputs_both.pdf','../../img/wpt_inputs_both.pdf');
