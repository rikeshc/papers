...
index2 = find(round(vreg_dcm_sch, 3) == 1.796, 1);
...
index2p = find(round(vreg_dcm_pex, 3) == 1.796, 1);
txt1p = sprintf('Vrec Min = %.2f V', dc_pex(index1p));
txt2p = sprintf('Vrec Min = %.2f V', dc_pex(index2p));

str1 = sprintf('Pre-layout, Vin min = %.2f V, Vreg = %.3f V', dc_sch(index2), mean(vreg_dcm_sch(1000:end)));
str2 = sprintf('Post-layout, Vin min = %.2f, Vreg = %.3f V', dc_pex(index2p), mean(vreg_dcm_pex(1000:end)));
