...
%xlim([-0.1, 2.5]); 
ylim([-.1, 2.5]);
title('WPT 1.65 mm: Vrec and Vreg', 'FontSize', 10);
...
rip_Vreg_pex = max(vreg_pex(fromp:end))-min(vreg_pex(fromp:end-100));
...
xlim([2000, 2000.15]);
%ylim([2.15, 2.25]);
title('WPT 1.65 mm: Rectified voltage, V_{rec}', 'FontSize', 10);
...
xlim([2000, 2000.15]);
%ylim([1.778, 1.783]);
title('WPT 1.65 mm: Regulated voltage, V_{reg}', 'FontSize', 10);
...
xlim([0, .16]);
%ylim([-5.6, 5.6]);
title('WPT 1.65 mm: Inputs voltages', 'FontSize', 10);
...
movefile('wpt_Vout_both.pdf','../../img/wpt_1.65/wpt_Vout_both.pdf');
%%
set(f2,'Units','Inches');
pos = get(f2,'Position');
set(f2,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(f2, 'wpt_ripple_both.pdf', '-dpdf');
movefile('wpt_ripple_both.pdf','../../img/wpt_1.65/wpt_ripple_both.pdf');
%%
set(f3,'Units','Inches');
pos = get(f3,'Position');
set(f3,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(f3, 'wpt_inputs_both.pdf', '-dpdf');
movefile('wpt_inputs_both.pdf','../../img//wpt_1.65/wpt_inputs_both.pdf');
