...
f1 = figure(1);
start = 2000.000;
from = find(start == round(time, 2), 1);
...
%ylim([-70, 70]);
title('WPT 1.65 mm: Current from source', 'FontSize', 10);
...
xlim([start, start + 0.15]);
%ylim([0, 35]);
title('WPT 1.65 mm: Power from source', 'FontSize', 10);
...
f2 = figure(2);
subplot(2, 1, 1);
p3 = plot(time(from:to), Ivin2_sch(from:to)*10^3, 'r'); hold on;
     plot(time(from:to), Ivin1_sch(from:to)*10^3, 'b');
     plot(timep(fromp:top), Ivin2_pex(fromp:top)*10^3, 'm-.');
     plot(timep(fromp:top), Ivin1_pex(fromp:top)*10^3, 'c-.');
...
xlim([start, start + 0.15]);
ylim([-3, 3]);
title('WPT 1.65 mm: Rectifier voltages and currents', 'FontSize', 10);
...
title('WPT 1.65 mm: Voltage and Current at primary', 'FontSize', 10);
...
movefile('wpt_PIsrc_both.pdf','../../img/wpt_1.65/wpt_PIsrc_both.pdf');
%%
set(f2,'Units','Inches');
pos = get(f2,'Position');
set(f2,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(f2, 'wpt_VIrect_both.pdf', '-dpdf');
movefile('wpt_VIrect_both.pdf','../../img/wpt_1.65/wpt_VIrect_both.pdf');
